...
janela.minsize(width=800, height=700)
...
def corrigir_data(data_str):
    if re.match(r'^\d{4}/\d{2}/\d{2}$', data_str):
        ...
        messagebox.showerror("Data inválida", "Por favor, insira a data no formato yyyy/mm/dd.")
        return False
...
def menu_inicial():
    ...
    bt_mudar_emprestimo = Button(frame_1, command=mudar_emprestimo, text= 'Emprestimo', bg = '#107db2', fg = 'white', font= ("verdana", 13, "bold"))
    bt_mudar_emprestimo.place(relx=0.15, rely=0.7, relwidth=0.75, relheight=0.15)
...
def emprestimo():
    ...
    def item_selecionado(event):
        for selected_item in lista_pos.selection():
            ...
            pos_mat_mov_entry.insert(0, record[0] )
            qnt_min_entry.insert(0, record[1] )
            qnt_atual_entry.insert(0, record[2] )
            material_entry.insert(0, record[4] )
    ...
    def buscar_emprestimo():
        try:
            cursor = conexao.cursor()
            texto_digitado = cod_emprestimo_entry.get()
            cursor.execute("SELECT COUNT(*) FROM movimentacao WHERE numero_mov = %s", (texto_digitado,))
            count = cursor.fetchone()[0]
            if count > 0:  
                consulta_sql = "SELECT * FROM movimentacao WHERE numero_mov = %s"
                ...
                for linha in resultados:
                    ...
                    qnt_mat_entry.insert(0, linha[1] )
                    data_emprestimo_entry.insert(0, linha[2] )
                    cod_prof_entry.insert(0, linha[3] )
                    tipo_mov = linha[4]
                    pos_mat_mov_entry.insert(0, linha[5] )
                    
                    if tipo_mov == "D": 
                        messagebox.showinfo("Mensagem!","Isto é uma devolução")
    ...
    bt_confirmar = Button(frame_1, text= 'Confirmar', bg = '#107db2', fg = 'white', font= ("verdana", 10, "bold"))
    bt_confirmar.place(relx=0.70, rely=0.08, relwidth=0.12, relheight=0.14)
    bt_confirmar.config(state='disabled')

    bt_cancelar = Button(frame_1,  text= 'Cancelar', bg = '#107db2', fg = 'white', font= ("verdana", 10, "bold"))
    bt_cancelar.place(relx=0.85, rely=0.08, relwidth=0.12, relheight=0.14)  
    bt_cancelar.config(state='disabled')
    
    lb_cod_emprestimo = Label(frame_1, text = "Código emp.", bg= '#dfe3ee', fg = '#107db2')
    lb_cod_emprestimo.place(relx= 0.02, rely= 0.05 )
    cod_emprestimo_entry = Entry(frame_1 )
    cod_emprestimo_entry.place(relx= 0.05, rely= 0.15, relwidth= 0.08)
    cod_emprestimo_entry.focus()
    
    lb_qnt_atual = Label(frame_1, text = "Qnt atual.", bg= '#dfe3ee', fg = '#107db2')
    lb_qnt_atual.place(relx= 0.05, rely= 0.67 )
    ...
    qnt_min_entry = Entry(frame_1 )
    qnt_min_entry.place(relx= 0.17, rely= 0.75, relwidth= 0.1)
    qnt_min_entry.config(state='disabled')
    
    lb_material= Label(frame_1, text = "Material.", bg= '#dfe3ee', fg = '#107db2')
    lb_material.place(relx= 0.29, rely= 0.67 )
    material_entry = Entry(frame_1 )
    material_entry.place(relx= 0.29, rely= 0.75, relwidth= 0.1)
    material_entry.config(state='disabled')
    
    lb_qnt_mat = Label(frame_1, text="Quantidade Material", bg= '#dfe3ee', fg = '#107db2')
    lb_qnt_mat.place(relx=0.05, rely=0.27)
    qnt_mat_entry= Entry(frame_1)
    qnt_mat_entry.place(relx=0.05, rely=0.35, relwidth=0.4)
    qnt_mat_entry.config(state='disabled')
        
    lb_cod_prof = Label(frame_1, text="Cod.Professor", bg= '#dfe3ee', fg = '#107db2')
    lb_cod_prof.place(relx=0.5, rely=0.27)
    cod_prof_entry = Entry(frame_1)
    cod_prof_entry.place(relx=0.50, rely=0.35, relwidth=0.4)
    cod_prof_entry.config(state='disabled')

    lb_pos_mat_mov = Label(frame_1, text="Cod posição do material", bg= '#dfe3ee', fg = '#107db2')
    lb_pos_mat_mov.place(relx=0.05, rely=0.45)
    pos_mat_mov_entry = Entry(frame_1)
    pos_mat_mov_entry.place(relx=0.05, rely=0.55, relwidth=0.4) 
    pos_mat_mov_entry.config(state='disabled')

    lb_data_emprestimo = Label(frame_1, text="Data", bg= '#dfe3ee', fg = '#107db2')
    lb_data_emprestimo.place(relx=0.5, rely=0.45)
    data_emprestimo_entry = Entry(frame_1)
    data_emprestimo_entry.place(relx=0.5, rely=0.55, relwidth=0.4)
    data_emprestimo_entry.config(state='disabled')
    ...
    lista_pos.bind("<ButtonRelease-1>", item_selecionado)
